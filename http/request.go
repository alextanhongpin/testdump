...
func WriteRequest(r *http.Request) ([]byte, error) {
	...
	// Add new lines for better separation.
	b = normalize(b)
	b = append(b, []byte("\n\n")...)
...
func DumpRequest(r *http.Request) (*Dump, error) {
...
func formatRequestLine(req *http.Request) string {
	...
	return fmt.Sprintf("%s %s HTTP/%d.%d", cmp.Or(req.Method, "GET"),
