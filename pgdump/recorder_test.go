...
func TestRecorder(t *testing.T) {
	...
	var id int
	var name string
	if err := rec.QueryRowContext(ctx, "select * from users where id = $1", 1).Scan(&id, &name); err != nil {
		t.Fatal(err)
	}
	if id != 1 || name != "Alice" {
	...
	if err := rec.QueryRowContext(ctx, "select * from users where id = $1", 2).Scan(&id, &name); err != nil {
		t.Fatal(err)
	}
	if id != 2 || name != "Bob" {
